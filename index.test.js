...
test('test create message with blocks', () => {
  ...
  const message = createMessage();
  expect(message.blocks[2].text.text).toEqual('test');
});
...
test('test create message with attachments with multiple lines', () => {
  ...
  const message = createMessage();
  expect(message.attachments[0].fields[1].value).toEqual(
    'https://github.com/foo/github-actions-sandbox/actions/runs/103198369'
  );
});
